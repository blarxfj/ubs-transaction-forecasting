...
KEYWORDS = [r'cloud|backup|storage', r'gym|fitness|\bfit\b|urban|club|membership', r'insurance|policy|cover|safe', r'phone|contract|service bill', r'audio|member pass|\bpass\b', r'software|saas|productivity|\bprod\b|suite', r'video|media']
...
def assign_families(g):
    ...
    for i, row in enumerate(g.itertuples()):
        ...
        if 'service plan' in desc:
            evidence[i, 0] += 1.0
        if 'monthly plan' in desc and row.mcc not in MCC and row.mcc!=5812:
            evidence[i, 3] += .2
    amount = np.maximum(g.amount.to_numpy(), .1)
    distance = abs(np.log(amount[:, None]/amount[None, :]))
    kernel = np.exp(-.5*(distance/.035)**2)*(distance<.12)*eligible[:, None]*eligible[None, :]
...
def stats(t, a):
    ...
    if len(t)>=2:
        # Robust latent cadence and missing-event reconstruction. The period grid
        # reflects common subscription intervals rather than any fitted labels.
        periods=np.array([7.,14.,30.,30.4375,60.,60.875,90.,91.3125,180.,365.])
        gaps=np.diff(t)
        costs=[]
        for period in periods:
            steps=np.maximum(np.round(gaps/period),1)
            costs.append(np.median(abs(gaps/steps-period))/period + .12*np.mean(steps-1))
        period=periods[np.argmin(costs)]
        steps=np.maximum(np.round(gaps/period),1)
        index=np.r_[0,np.cumsum(steps)]
        slope=(t[-1]-t[0])/max(index[-1],1)
        residual=t-index*period
        next_phase=np.median(residual[-5:])+(index[-1]+1)*period
        out.update(cadence=period, cadence_error=min(costs), cadence_missing=float(np.sum(steps-1)), cadence_missing_fraction=float(np.mean(steps-1)),cadence_slope=slope, cadence_due=next_phase,cadence_last_due=t[-1]+period,cadence_active_due=next_phase if next_phase>=-7 else 999, cadence_age_ratio=age/period)
        for size in [3,5,8]:
            tt=t[-size:];ii=index[-size:]
            pp=np.polyfit(ii-ii[-1],tt,1)
            out[f'linear_due{size}']=float(pp[0]+pp[1])
            out[f'linear_period{size}']=float(pp[0])
            out[f'linear_error{size}']=float(np.std(tt-np.polyval(pp,ii-ii[-1])))
        for mult in [1.2,1.5,2.0]:
            out[f'active_n_{mult}']=len(t) if age<period*mult else 0
            out[f'active_due_{mult}']=max(next_phase,0) if age<period*mult else 999
    return out
...
def one_client(g):
    ...
    generic=g.description.isin(['monthly plan','member plan','subscription charge','digital service'])
    vague=g.description.isin(['card purchase','digital order','merchant charge','service payment'])
    out['generic_fraction']=generic.mean()
    out['vague_fraction']=vague.mean()
    out['assigned_fraction']=(fam>=0).mean()
    g['generic']=generic.to_numpy();g['vague']=vague.to_numpy()
    per=[]
    for k in range(7):
        ...
        if len(h):
            ...
            f['generic_fraction']=float(h.generic.mean())
            f['vague_fraction']=float(h.vague.mean())
            f['specific_fraction']=float(h.description.str.contains(KEYWORDS[k],regex=True).mean())
...
def matrices(z,compact=False):
    ...
    summary=np.concatenate([sorted_[:,:3].reshape(len(t),-1),np.nan_to_num(t[:,:7],nan=-999).max(axis=1)],axis=1)
    # Joint competitor features preserve which attributes belong to one stream.
    joint_names=['n','age','amount_cv','gap_med','core_n','core_age','core_amount_cv','core_cadence_due','core_cadence_error','cadence_due','cadence_error','core_active_due_1.5','refund_n']
    joint=t[:,:7,[names.index(v) for v in joint_names]]
    ...
    delta=t-sorted_[:,0:1,:]
    if compact:
        keys=joint_names+['core_active_n_1.5','cadence_age_ratio','core_cadence_age_ratio','core_fraction','n30','n90','text_confidence']
        keep=[names.index(k) for k in keys]
        low=sorted_[:,0,keep];high=np.nan_to_num(t[:,:7,keep],nan=-999).max(axis=1)
        context=np.concatenate([g,low,high,ordered[:,:4].reshape(len(t),-1)],axis=1)
        candidate=np.concatenate([t,rank[:,:,keep],np.repeat(context[:,None,:],8,axis=1),np.tile(np.eye(8)[None,:,:],(len(t),1,1))],axis=2)
    else:
...
def matrix_feature_names(z,compact=False):
    names=list(z['features']);global_keys=list(z['global_keys'])
    joint=['n','age','amount_cv','gap_med','core_n','core_age','core_amount_cv','core_cadence_due','core_cadence_error','cadence_due','cadence_error','core_active_due_1.5','refund_n']
    result=['own_'+f for f in names]
    if compact:
        keys=joint+['core_active_n_1.5','cadence_age_ratio','core_cadence_age_ratio','core_fraction','n30','n90','text_confidence']
        result+=['rank_'+f for f in keys]+['global_'+f for f in global_keys]
        result += [prefix+f for prefix in ['minimum_','maximum_'] for f in keys]
        result += [f'competitor{i}_'+f for i in range(4) for f in joint]
    else:
        result+=['rank_'+f for f in names]+['delta_from_minimum_'+f for f in names]
        result+=['global_'+f for f in global_keys]
        result += [prefix+f for prefix in ['minimum1_','minimum2_','minimum3_','maximum_'] for f in names]
        result += [f'competitor{i}_'+f for i in range(4) for f in joint]
    return result+['family_'+label for label in LABELS]
