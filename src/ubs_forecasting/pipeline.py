...
from .features import NOISE_LEVEL_FEATURES, build_features
...
EVALUATION_TABLES = ("train", "valid", "trainT", "trainV", "validT0", "validT1")
...
def evaluate_tables(
    ...
    *,
    ...
    """Run rule checks, the leak regression, and held-out valid CV."""
    ...
    valid_all, _ = fit_predict([(tables["train"], train_labels)], [tables["valid"], tables["validT0"]])
    valid_safe, test_noise_safe = fit_predict(
        [(tables["train"], train_labels)],
        [tables["valid"], tables["validT0"]],
        drop=NOISE_LEVEL_FEATURES,
    )
    leak = {
    ...
    seed_valid: list[pd.DataFrame] = []
    seed_test_noise: list[pd.DataFrame] = []
    for fold_seed in fold_seeds:
        out_of_fold_valid: list[pd.DataFrame] = []
        out_of_fold_test_noise: list[pd.DataFrame] = []
        ...
        for train_indices, heldout_indices in splitter.split(client_ids, valid_labels.values):
            ...
            valid_probabilities, test_noise_probabilities = fit_predict(
                training,
                [
                    tables["valid"][tables["valid"].client_id.isin(heldout_clients)],
                    tables["validT0"][tables["validT0"].client_id.isin(heldout_clients)],
                ],
            )
            out_of_fold_valid.append(valid_probabilities)
            out_of_fold_test_noise.append(test_noise_probabilities)
        seed_valid.append(pd.concat(out_of_fold_valid).reindex(valid_labels.index))
        seed_test_noise.append(pd.concat(out_of_fold_test_noise).reindex(valid_labels.index))
    valid_probabilities = sum(seed_valid[1:], seed_valid[0].copy()) / len(seed_valid)
    test_noise_probabilities = sum(seed_test_noise[1:], seed_test_noise[0].copy()) / len(seed_test_noise)
    valid_predictions = decide(valid_probabilities)
    test_noise_predictions = decide(test_noise_probabilities)
    ...
    test_noise_metrics = metric_report(
        valid_labels,
        test_noise_predictions,
        bootstrap_samples=bootstrap_samples,
        bootstrap_seed=2027,
    )
    print(format_metric_line("CV -> valid  (valid noise)", valid_metrics), flush=True)
    print(format_metric_line("CV -> validT (test-level noise)", test_noise_metrics), flush=True)
    valid_auc = none_auc(valid_labels, valid_probabilities)
    test_noise_auc = none_auc(valid_labels, test_noise_probabilities)
    print(f"none-gate AUC valid={valid_auc:.4f} validT={test_noise_auc:.4f}", flush=True)
    result = {
        "protocol": {
            "folds": 5,
        ...
        "valid_test_noise": {**test_noise_metrics, "none_auc": test_noise_auc},
    }
    return result, valid_probabilities, test_noise_probabilities
...
def train_final_model(
    ...
    return fit_models(training, amount_prior=amount_prior)
...
def render_evaluation_markdown(evaluation: dict[str, Any]) -> str:
    """Render the two held-out noise conditions with bootstrap intervals."""
    ...
    lines = [
        ...
        (
            ...
            "both the original-valid and additionally noised training copies. Intervals are "
            f"{int(100 * evaluation['valid']['bootstrap']['confidence'])}% client-bootstrap percentile "
            f"intervals ({protocol['bootstrap_samples']:,} resamples)."
        ),
        "",
    ...
    lines.extend(
        [
            ...
            "uses those useful features only after label-independent re-noising.",
            "",
        ]
    )
    return "\n".join(lines)
